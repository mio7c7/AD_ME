...
def evaluate_detection(ground_truth, flagged, error_margin):
    ...
    for i in range(num_change):
        ...
        if cp[i]:
            ...
            not_detected += 1
        # FA
        FA += cp[margin:end_ind].sum()

    results = {'EDD': EDD / np.max((num_change - not_detected, 1)),
               'not_detected': not_detected,
               'false_alarm': FA, 'cp': cp, 'num_cp': num_change}
    return results
...
def compute_curves(ground_truth, dist,
    ...
    for i in range(num_points):
        ...
        res = evaluate_detection(ground_truth, flagged_points, error_margin)
        EDDs[i] = res['EDD']
        FAs[i] = res['false_alarm']
        NDs[i] = res['not_detected']
    CP = res['num_cp']
    return EDDs, FAs, NDs, CP
