...
def objective_function(params, mus, sigmas, epoch_sizes, M, k, calculate_grad=True, loss_translation=0):
    ...
    gradient = np.sum([(-a.dot(b) + c.reshape(params['dimensions_stationary'], 1).dot(
        d.reshape(1, params['dimensions_total']))) * epoch_size
...
def optimize_once(data, params, optNSources, loss_translation=0, init=None):
    ...
    for i in range(maxiter):
        loss, grad, _, _, _ = objective_function(params, mus, sigmas, data['epoch_sizes'], None, k,
        ...
        for i in range(10):  # Line search
            M = t * search
            loss_new, _, rotation, mus_new, sigmas_new = objective_function(params, mus, sigmas, data['epoch_sizes'], M,
                                                                            k, calculate_grad=False,
                                                                            loss_translation=loss_translation)
            if optNSources:
                loss_new *= -1
...
def optimize(data, dim_s, restarts=10, optNSources=False, loss_translation=0, init=None):
    ...
    try:
        ...
        t = loss.args[0]
        raise ValueError(
            "The un-normalized loss function returned {}. This shouldn't happen, but a possible fix could be to run the optimizer again, and supply a positive argument to loss_translation".format(
                t))
