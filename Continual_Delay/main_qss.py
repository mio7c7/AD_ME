...
parser.add_argument('--dense_dim', type=int, default=32, help='no of neuron in dense')
...
parser.add_argument('--outfile', type=str, default='qs100_400_5_9_5mean', help='name of file to save results')
...
if __name__ == '__main__':
    ...
    for i in glob.glob(folder):
        if i in ignored:
            continue
        if i != '../data3\\A073_T1bottom02.npz':
            continue
        ...
        while ctr < test_var_dl.shape[0]:
            ...
            if collection_period > args.min_requirement:
                ...
                for aa in range(len(z_mean)):
                    ...
                    ms = np.mean(scores[-args.cp_range:])
                    mss.append(ms)
