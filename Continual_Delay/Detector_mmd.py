...
class Detector():
    ...
    def __init__(self, window_size, pca, args):
    ...
    def resample(self, new_sample):
        ...
        sam = org
        rep = self.pca.transform(sam)
        self.memory[self.current_index]['centroid'] = np.mean(rep, axis=0)
        self.current_centroid = self.memory[self.current_index]['centroid']
        self.memory_info[self.current_index]['threshold'] = self.compute_threshold(rep, self.current_centroid, threshold)
