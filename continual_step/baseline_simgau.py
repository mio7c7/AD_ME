...
parser.add_argument('--min_requirement', type=int, default=30, help='window size')
parser.add_argument('--memory_size', type=int, default=10, help='memory size per distribution ')
parser.add_argument('--cp_range', type=int, default=1, help='range to determine cp')
...
parser.add_argument('--quantile', type=float, default=0.75, help='quantile')
...
if __name__ == '__main__':
    WINDOW_SIZE = 100
    np.random.seed(100)
    timeseries, windows_TD, parameters = generate_gaussian(WINDOW_SIZE)
    ...
    ax1.plot(thresholds, color='tab:purple', alpha=0.5, label='filtered_zmean')
