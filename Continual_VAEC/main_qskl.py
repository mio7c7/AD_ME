...
from Detector_kl import Detector
...
parser.add_argument('--outfile', type=str, default='kl', help='name of file to save results')
...
if __name__ == '__main__':
    ...
    for i in glob.glob(folder):
        ...
        kernel_matrix = kpca.fit(X_train)
        # centered_kernel_matrix = kernel_matrix - np.mean(kernel_matrix, axis=0)
        n_components = 1
        ...
        kernel_matrix = kpca.transform(memory)
        ...
        detector = Detector(args.ws, kpca, args)
        detector.addsample2memory(memory, representation, class_no, len(memory))
        detector.feature_extracter = kpca
        ...
        while ctr < test_var_dl.shape[0]:
            ...
            if collection_period > args.min_requirement:
                ...
                kernel_matrix = detector.feature_extracter.transform(window)
                kernel_matrix = kernel_matrix - np.mean(kernel_matrix, axis=0)
                eigenvalues, eigenvectors = np.linalg.eig(kernel_matrix.T.dot(kernel_matrix))
                selected_eigenvectors = eigenvectors[:, :n_components]
                representation = kernel_matrix.dot(selected_eigenvectors)

                for aa in range(len(representation)):
                    score = euclidean(representation[aa], detector.current_centroid)
                    scores.append(score)
                    ...
                    if ms > detector.memory_info[detector.current_index]['threshold']:
                        ...
                        while n <= len(detector.memory):
                            ...
                            try:
                                ...
                                kernel_matrix = detector.feature_extracter.transform(mm)
                                kernel_matrix = kernel_matrix - np.mean(kernel_matrix, axis=0)
                                eigenvalues, eigenvectors = np.linalg.eig(kernel_matrix.T.dot(kernel_matrix))
                                selected_eigenvectors = eigenvectors[:, :n_components]
                                representation = kernel_matrix.dot(selected_eigenvectors)
                                distance = np.median([euclidean(representation[i], detector.current_centroid) for i in range(len(representation))])
                            except:
                                distance = euclidean(representation[aa], detector.current_centroid)
                        ...
                        collection_period = 0
                        detected = True
                        filtered = filtered[:-len(representation) + aa + 1]
                        detector.newsample = []
                        break
                    else:
                ...
                if collection_period + len(window) <= args.min_requirement:
                    ...
                    if detector.current_index == -1: # new cluster
                        exist_data = np.empty((0, reconstructeds.shape[1]))
                        jj = 1
                        while jj <= class_no:
                            exist_data = np.vstack((exist_data, detector.memory[jj]['sample']))
                            jj += 1
                        new_train, new_valid = train_test_split(sample, test_size=0.5, shuffle=True, random_state=0)
                        train = np.concatenate((new_train, new_train, new_train, exist_data))
                        detector.feature_extracter.fit(train)
                        kernel_matrix = detector.feature_extracter.transform(new_valid)
                        eigenvalues, eigenvectors = np.linalg.eig(kernel_matrix.T.dot(kernel_matrix))
                        selected_eigenvectors = eigenvectors[:, :n_components]
                        representation = kernel_matrix.dot(selected_eigenvectors)
                        class_no += 1
                        detector.addsample2memory(new_valid, representation, class_no, len(new_valid))
                    else: # recurring
                        new_train, new_valid = train_test_split(sample, test_size=0.5, shuffle=True, random_state=0)
                        org = detector.memory[detector.current_index]['sample']
                        others = np.empty((0, reconstructeds.shape[1]))
                        jj = 1
                        while jj <= class_no:
                            ...
                            jj += 1
                        train = np.concatenate((new_train, org, others))
                        detector.feature_extracter.fit(train)
                        kernel_matrix = detector.feature_extracter.transform(new_valid)
                        kernel_matrix = kernel_matrix - np.mean(kernel_matrix, axis=0)
                        eigenvalues, eigenvectors = np.linalg.eig(kernel_matrix.T.dot(kernel_matrix))
                        selected_eigenvectors = eigenvectors[:, :n_components]
                        representation = kernel_matrix.dot(selected_eigenvectors)
                        detector.updaterecur(new_valid, representation)
                    collection_period = 1000000000
                    sample = np.empty((0, args.ws))
